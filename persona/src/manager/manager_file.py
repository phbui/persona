    ...
    def upload_file(self, file_path):
        ...
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                data = json.load(file)
            self.logger.add_log("INFO", "uploads", "Manager_File", "upload_file", f"Data successfully uploaded from {file_path}")
            return data
        except Exception as e:
            self.logger.add_log("ERROR", "uploads", "Manager_File", "upload_file", f"Error reading JSON from {file_path}: {e}")
            return None
    ...
    def download_file(self, data, dir_path, filename="downloaded.json"):
        ...
        if not os.path.exists(dir_path):
            try:
                ...
                self.logger.add_log("INFO", "downloads", "Manager_File", "download_file", f"Directory {dir_path} created.")
            except Exception as e:
                self.logger.add_log("ERROR", "downloads", "Manager_File", "download_file", f"Error creating directory {dir_path}: {e}")
                return False
        ...
        try:
            ...
            self.logger.add_log("INFO", "downloads", "Manager_File", "download_file", f"Data successfully downloaded to {file_path}")
            return True
        except Exception as e:
            self.logger.add_log("ERROR", "downloads", "Manager_File", "download_file", f"Error writing JSON to {file_path}: {e}")
            return False